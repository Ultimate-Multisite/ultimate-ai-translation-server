...
`POST /wp-json/sd-ai-lang-pack/v1/request-translation`
...
`POST /wp-json/sd-ai-lang-pack/v1/batch-check-translations`

`{"core":{"version":"7.1"},"plugins":[...],"themes":[...],"locales":["de_DE"]}`

Core results are namespaced as `core:wordpress`; existing plugin and theme result keys and payloads are unchanged. A client that receives no core result from an older server can safely continue with its existing plugin/theme flow. Caller-supplied `auto_approve`, plus the `/approve` and `/reject` controls, are limited to server administrators; public clients use the normal request-and-approval flow.
